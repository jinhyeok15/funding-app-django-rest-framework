...
* ShopPurchaseLog에는 User와 Item object, status=PURCHASE/DONATE/CANCEL/CLOSE, updated 컬럼이 존재한다. PURCHASE는 펀딩이 성공적으로 진행되어 구매단계까지 진행된 상태이며, DONATE는 펀딩에 참여하였으나 마감일이 끝나지 않은 상태, CANCEL은 펀딩을 취소한 상태, CLOSE는 펀딩 목표 금액을 넘지 못하여 펀딩이 취소된 상태를 의미한다. 결제는 PURCHASE단계에서 진행되며, CANCEL이 되면 결제 내역이 환불된다.
